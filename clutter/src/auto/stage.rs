...
pub trait StageExt: 'static {
    ...
    //fn capture_view_into(&self, view: /*Ignored*/&StageView, rect: &mut cairo::RectangleInt, data: u8, stride: i32);
    ...
    fn capture_final_size(&self, rect: &mut cairo::RectangleInt) -> Option<(i32, i32, f32)>;
...
impl<O: IsA<Stage>> StageExt for O {
    ...
    fn capture_final_size(&self, rect: &mut cairo::RectangleInt) -> Option<(i32, i32, f32)> {
        unsafe {
            let mut out_width = mem::MaybeUninit::uninit();
            let mut out_height = mem::MaybeUninit::uninit();
            let mut out_scale = mem::MaybeUninit::uninit();
            let ret = from_glib(ffi::clutter_stage_get_capture_final_size(self.as_ref().to_glib_none().0, rect.to_glib_none_mut().0, out_width.as_mut_ptr(), out_height.as_mut_ptr(), out_scale.as_mut_ptr()));
            let out_width = out_width.assume_init();
            let out_height = out_height.assume_init();
            let out_scale = out_scale.assume_init();
            if ret { Some((out_width, out_height, out_scale)) } else { None }
        }
    }
