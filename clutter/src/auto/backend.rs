...
impl Backend {
    ...
    ///  The returned [`cairo::FontOptions`][crate::cairo::FontOptions] is owned by the backend and should
    ...
    pub fn font_options(&self) -> Option<cairo::FontOptions> {
        unsafe {
            from_glib_none(ffi::clutter_backend_get_font_options(self.to_glib_none().0))
        }
    }
    ...
    /// [`pango::FontDescription`][crate::pango::FontDescription] and cairo units. The default value is 96.0,
    ...
    /// copy the [`cairo::FontOptions`][crate::cairo::FontOptions].
    ///
    /// If `options` is [`None`], the first following call to
    /// [`font_options()`][Self::font_options()] will return the default font
    /// options for `self`.
    ///
